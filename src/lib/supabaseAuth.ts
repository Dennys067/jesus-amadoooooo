...
export const initAuth = (
  ...
  const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
    if (session && session.user) {
      ...
      // Use the provider token (Google Access Token) if available, or fallback to session token
      ...
      // Make sure we didn't just set a mock user
...
export const googleSignIn = async (): Promise<{ user: GoogleUser; accessToken: string } | null> => {
  ...
  /*if (!isSupabaseConfigured || !supabase) {
    ...
    const mockUser: GoogleUser = {
      uid: 'guest-admin',
      email: 'proprietario@arenaprime.com',
      displayName: 'Proprietário Arena Prime',
